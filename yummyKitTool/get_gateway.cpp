...
void printRoute(struct route_info *rtInfo) {
    ...
    if(rtInfo->dstAddr.s_addr != 0) strcpy(tempBuf,  inet_ntoa(rtInfo->dstAddr));
    else sprintf(tempBuf, "*.*.*.*\t");
//    fprintf(stdout, "%s\t", tempBuf);

    // Print Gateway address
    if(rtInfo->gateWay.s_addr != 0) strcpy(tempBuf, (char *) inet_ntoa(rtInfo->gateWay));
    else sprintf(tempBuf, "*.*.*.*\t");
//    fprintf(stdout, "%s\t", tempBuf);
...
//    printf("%s\n", inet_ntoa(rtInfo->dstAddr));
    ...
    if(rtInfo->dstAddr.s_addr == 0) sprintf(gateway, (char *) inet_ntoa(rtInfo->gateWay));
//    printRoute(rtInfo);

    return;
}

void get_gateway::get_route_ip(char *route) {
